...
#
# Setup and usage:
# 1) Create a sqlite3 database called accounts.db with a table called accounts created using CREATE TABLE accounts (email TEXT, password TEXT, data TEXT);
# 2) Load email/password combinations into the table; data field is to be left empty and will be populated by reusable.py later.
# 3) Put reusable.py and accounts.db in a directory called reusable in your $HOME, and run reusable.py periodically with this cronjob entry:
#    * * * * * cd ~/reusable; python reusable.py >> reusable.log 2>&1
#    While this entry will try to run reusable.py every minute, only MAX_INSTANCES of reusable.py will actually run at any one time.
#
...
# Limit up to this number of instances of this script to run concurrently at any one time; all of them will access the same sqlite3 database, accounts.db
MAX_INSTANCES = 4
...
for account in accounts:
    ...
    if '/logout' in page:
        ...
        # Twitter screen name for this account
        screen_name = re.findall('"currentUserScreenName"\:"(?P<screen_name>\w+)"', page)[0]

        data = '{"avatar": "%s", "following": "%d", "followers": "%d", "screen_name": "%s"}' % (avatar, following, followers, screen_name)
